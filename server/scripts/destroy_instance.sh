...
# AdAgent Instance Destruction Script
...
find_instance_by_name() {
    ...
    if [ "$INSTANCE_ID" = "None" ] || [ -z "$INSTANCE_ID" ]; then
        print_error "No instance found with name tag: $name"
        return 1
    fi
...
main() {
    ...
    if [ -z "$identifier" ]; then
        ...
        echo
        echo "Usage: $0 <instance-name-or-ip>"
        echo "   or: $0 --list"
        echo
        echo "Examples:"
        echo "  $0 adagent-server-dev"
        echo "  $0 3.80.73.248"
        echo "  $0 --list"
        exit 1
    fi
    ...
    if ! find_instance_by_name "$identifier"; then
        if ! find_instance_by_ip "$identifier"; then
            ...
            echo
            print_status "Use '$0 --list' to see available instances"
            exit 1
        fi
    fi
...
# Check for help
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    echo "AdAgent Instance Destruction Script"
    echo
    echo "Usage: $0 <instance-name-or-ip>"
    echo "   or: $0 --list"
    echo
    echo "This script will:"
    echo "1. Find the specified instance by name tag or IP address"
